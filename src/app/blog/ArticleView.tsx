    ...
    return (
      <div className={`min-h-screen ${isDark ? "bg-gray-900 text-white" : "bg-gray-50 text-gray-900"}`}>
        ...
        <div className="max-w-3xl mx-auto px-4 py-16 text-center">
          ...
          <Link href="/blog" className="text-blue-400 hover:underline text-sm">Ver todos os artigos</Link>
        </div>
      </div>
    );
  }
    ...
    <div className={`min-h-screen ${isDark ? "bg-gray-900 text-white" : "bg-gray-50 text-gray-900"}`}>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          <Link href="/blog" className={`flex items-center gap-2 text-sm transition-colors ${isDark ? "text-gray-400 hover:text-white" : "text-gray-500 hover:text-gray-900"}`}>
            <ArrowLeft className="w-4 h-4" />
            Todos os artigos
          </Link>
          <button onClick={handleShare} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${isDark ? "hover:bg-white/5 text-gray-400 hover:text-white" : "hover:bg-gray-200 text-gray-600"}`}>
        ...
        <div className={`flex flex-wrap items-center gap-3 text-xs mb-4 ${descClass}`}>
          ...
          {article.tone && (
            <span className={`px-2 py-0.5 rounded-full ${isDark ? "bg-white/5" : "bg-gray-100"}`}>{TONE_LABELS[article.tone] || article.tone}</span>
          )}
          {article.length && (
            <span className={`px-2 py-0.5 rounded-full ${isDark ? "bg-white/5" : "bg-gray-100"}`}>{LENGTH_LABELS[article.length] || article.length}</span>
          )}
        </div>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {article.tags.map((tag) => (
              <Link key={tag} href={`/blog?tag=${encodeURIComponent(tag)}`}
                className={`flex items-center gap-1 text-xs px-2.5 py-1 rounded-full transition-colors ${isDark ? "bg-blue-500/15 text-blue-400 hover:bg-blue-500/25" : "bg-blue-100 text-blue-700 hover:bg-blue-200"}`}>
        ...
        <div className={`mt-8 pt-6 border-t ${isDark ? "border-white/10" : "border-gray-200"} flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4`}>
          <p className={`text-xs ${descClass}`}>Gerado com base em documentos do sistema CCT</p>
          <Link href="/article" className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white text-xs font-medium transition-all">
