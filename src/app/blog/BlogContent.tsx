      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/" className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${isDark ? "hover:bg-white/5 text-gray-400" : "hover:bg-gray-200 text-gray-600"}`}>
              Chat
            </Link>
          </div>
        </div>
        ...
        {(activeTag || activeSearch) && (
          <div className="flex flex-wrap items-center gap-2 mb-5">
            ...
            {activeTag && (
              <button onClick={() => handleTagClick(activeTag)} className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-500/20 text-blue-400 text-xs hover:bg-blue-500/30 transition-colors">
            ...
            {activeSearch && (
              <button onClick={clearSearch} className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs transition-colors ${isDark ? "bg-white/10 text-gray-300 hover:bg-white/20" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}>
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className={`rounded-2xl border p-5 animate-pulse ${isDark ? "bg-gray-800/40 border-gray-700/50" : "bg-gray-100 border-gray-200"}`}>
          ...
          <div className={`rounded-2xl border p-12 flex flex-col items-center justify-center text-center ${isDark ? "bg-gray-800/40 border-gray-700/50" : "bg-white border-gray-200"}`}>
            ...
            <p className={`text-lg font-medium mb-2 ${isDark ? "text-white" : "text-gray-900"}`}>
              {activeSearch ? `Nenhum resultado para "${activeSearch}"` : activeTag ? `Nenhum artigo com a tag "${activeTag}"` : "Nenhum artigo publicado ainda"}
            </p>
            <p className={`text-sm ${descClass}`}>
              <Link href="/article" className="text-blue-400 hover:underline">Gere o primeiro artigo</Link>
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {articles.map((art) => (
              <Link key={art.id} href={`/blog/${art.id}`} className={`rounded-2xl border p-5 flex flex-col gap-3 transition-all ${cardClass} group`}>
                ...
                {art.tags?.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {art.tags.slice(0, 3).map((tag) => (
                      <span key={tag} onClick={(e) => { e.preventDefault(); handleTagClick(tag); }}
                        className={`text-xs px-2 py-0.5 rounded-full cursor-pointer transition-colors ${activeTag === tag ? "bg-blue-500 text-white" : isDark ? "bg-blue-500/15 text-blue-400 hover:bg-blue-500/25" : "bg-blue-100 text-blue-700 hover:bg-blue-200"}`}>
                        {tag}
                      </span>
                    ))}
                    {art.tags.length > 3 && (
                      <span className={`text-xs px-2 py-0.5 rounded-full ${isDark ? "bg-white/5 text-gray-500" : "bg-gray-100 text-gray-500"}`}>+{art.tags.length - 3}</span>
                    )}
                  </div>
                )}
                <div className={`flex items-center justify-between text-xs ${descClass}`}>
                  ...
                  {art.tone && (
                    <span className={`px-2 py-0.5 rounded-full ${isDark ? "bg-white/5" : "bg-gray-100"}`}>{TONE_LABELS[art.tone] || art.tone}</span>
                  )}
                </div>
              </Link>
            ))}
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-2 mt-8">
            <button onClick={() => handlePageChange(page - 1)} disabled={page === 1} className={`p-2 rounded-lg transition-colors disabled:opacity-40 ${isDark ? "hover:bg-white/5 text-gray-400" : "hover:bg-gray-200 text-gray-600"}`}>
              <ChevronLeft className="w-5 h-5" />
            </button>
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
              <button key={p} onClick={() => handlePageChange(p)}
                className={`w-9 h-9 rounded-lg text-sm font-medium transition-colors ${p === page ? "bg-blue-500 text-white" : isDark ? "hover:bg-white/5 text-gray-400" : "hover:bg-gray-200 text-gray-600"}`}>
                {p}
              </button>
            ))}
            <button onClick={() => handlePageChange(page + 1)} disabled={page === totalPages} className={`p-2 rounded-lg transition-colors disabled:opacity-40 ${isDark ? "hover:bg-white/5 text-gray-400" : "hover:bg-gray-200 text-gray-600"}`}>
